...
export default function AuthModal({ onClose, initialMode = 'login' }) {
  ...
  const [mode, setMode] = useState(initialMode === 'forgot-password' ? 'login' : initialMode); // 'login' | 'signup'
  ...
  const handleDemoGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      const demoName = prompt('Enter your full name for Google OAuth session:', 'Rahul Kiran');
      if (!demoName) {
        setLoading(false);
        return;
      }
      ...
      showToast(`Welcome ${demoName}! Signed in via Google.`, 'success');
      onClose();
      navigate('/dashboard', { replace: true });
    } catch (err) {
      setError(err.message || 'Google OAuth failed.');
      showToast(err.message || 'Google OAuth failed.', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                TruthLens Enterprise Security Suite
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: mode === 'signup' ? '16px' : '22px' }}>
            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-secondary)', marginBottom: '6px' }}>
              Password
            </label>
            <div style={{ position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: '20px', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          ...
          <button
            ...
            style={{ color: 'var(--apple-accent-blue)', fontWeight: 800, background: 'none', border: 'none', cursor: 'pointer' }}
          >
